...
use crate::{api::models::{PaginateData, SourceThemePatch, SourceThemesResp, UsersPatch}, db::{news::news::{delete_sources_db, get_sources_db, insert_sources_db, update_source_themes_db, update_sources_db}, user::user::{delete_users_db, get_all_roles_db, get_source_themes, get_users_db, update_users_db}}, error::ApiError, jwt_auth, setup::AppState};
...
pub fn admin_router(state: Arc<AppState>) -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route_layer(middleware::from_fn_with_state(state.clone(), jwt_auth::auth))
}
