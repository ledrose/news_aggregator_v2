...
use super::models::{NewsReturn, SearchOptions};
...
pub async fn news(State(state): State<Arc<AppState>>,Json(mut news_batch): Json<NewsBatchInfo>) -> Result<impl IntoResponse,ApiError> {
    ...
    let news = conn.interact(move |conn| {
        get_news(news_batch.max_id, news_batch.offset, news_batch.amount, &mut news_batch.prefs, conn)
    }).await?;
    Ok(Json(NewsReturn { max_id: news.1, news: news.0 }))
}
